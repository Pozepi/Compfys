...
{   
   ...
   switch(problem) 
   {
      case 2:
      {
         ...
         double * u_filled = Poisson_analytical(x, u_empty, length);
         std::string filename = "values1";
         write_to_file(filename, u_filled, x, length);
      ...
      case 8:
      {
         ...
         double * u; 
         u = new double;
         u = Poisson_analytical(x, lin, length);
      ...
      case 9:
      {
         ...
         for (int i=0;i<Nn;i++)
         {
            ...
            double lins[length]; // dummy array to be filled and abused
            double * lin = fill_array(lins, length, 0);
            double * x = linspace(start, end, lin, length);
