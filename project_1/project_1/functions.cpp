...
double * Poisson_analytical(double x[], double u[], int n)
{   
    for (int i=0; i < n; i++)
    {
        u[i] = 1 - (1-expf(-10)) * x[i] - expf(-10 * x[i]);
    }
    return u;
}
...
double * Delta(double u[], double v[], double error[], int n)
{   
    for (int i=0; i<n; i++)
    {
        error[i] = abs(u[i]-v[i]);
...
double * epsilon(double u[], double v[], double eps[], int n)
{
    for (int i=0; i<n; i++)
    {
        eps[i] = abs((u[i]-v[i])/u[i]);
    }
    return eps;
}
